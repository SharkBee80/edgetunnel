...
let PIP = ''; 
let sub = '';
let subConver = 'SUBAPI.fxxk.dedyn.io'; 
...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			PID = env.UUID || env.uuid || env.PASSWORD || env.pswd || PID;
			if (env.KEY || env.TOKEN || (PID && !isValidUUID(PID))) {
				FlowPIP = env.KEY || env.TOKEN || PID;
				enabletime = Number(env.TIME) || enabletime;
				updatetime = Number(env.UPTIME) || updatetime;
				const userIDs = await creatFlowPIP(FlowPIP);
				PID = userIDs[0];
				userIDLow = userIDs[1];
			}
			...
			const fakeUserIDMD5 = await 双重哈希(`${PID}${timestamp}`);
			fakePID = [
			...
			PIP = env.PROXYIP || env.Pip || PIP;
			PIPs = await 整理(PIP);
			PIP = PIPs[Math.floor(Math.random() * PIPs.length)];

			S5Add = env.SOCKS5 || S5Add;
			S5s = await 整理(S5Add);
			S5Add = S5s[Math.floor(Math.random() * S5s.length)];
			S5Add = S5Add.split('//')[1] || S5Add;
			if (env.GO2SOCKS5) go2S5s = await 整理(env.GO2SOCKS5);
			if (env.CFPORTS) httpsPorts = await 整理(env.CFPORTS);
			if (env.BAN) banHosts = await 整理(env.BAN);
			if (S5Add) {
				try {
					parsedS5Add = socks5AddressParser(S5Add);
					RPIP = env.RPROXYIP || 'false';
					enableS = true;
				} catch (err) {
					let e = err;
					console.log(e.toString());
					RPIP = env.RPROXYIP || !PIP ? 'true' : 'false';
					enableS = false;
				}
			} else {
				RPIP = env.RPROXYIP || !PIP ? 'true' : 'false';
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				if (env.ADD) add = await 整理(env.ADD);
				if (env.ADDAPI) addapi = await 整理(env.ADDAPI);
				if (env.ADDNOTLS) addnotls = await 整理(env.ADDNOTLS);
				if (env.ADDNOTLSAPI) addnotlsapi = await 整理(env.ADDNOTLSAPI);
				if (env.ADDCSV) addcsv = await 整理(env.ADDCSV);
				...
				sub = env.SUB || sub;
				subConver = env.SUBAPI || subConver;
				if (subConver.includes("http://") ){
					subConver = subConver.split("//")[1];
					subProtocol = 'http';
				} else {
					subConver = subConver.split("//")[1] || subConver;
				}
				subConfig = env.SUBCONFIG || subConfig;
				if (url.searchParams.has('sub') && url.searchParams.get('sub') !== '') sub = url.searchParams.get('sub');
				if (url.searchParams.has('notls')) noTLS = 'true';

				if (url.searchParams.has('Pip')) {
					path = `/?ed=2560&Pip=${url.searchParams.get('Pip')}`;
					RPIP = 'false';
				...
				if (路径 == '/') {
					...
					const fakeConfig = await 生成配置信息(PID, request.headers.get('Host'), sub, 'CF-Workers-SUB', RPIP, url, env);
					return new Response(`${fakeConfig}`, { status: 200 });
				} else if (url.pathname == `/${FlowPIP}/edit` || 路径 == `/${PID}/edit`) {
					const html = await KV(request, env);
					return html;
				} else if (url.pathname == `/${FlowPIP}` || 路径 == `/${PID}`) {
					await sendMessage(`#获取订阅 ${FileName}`, request.headers.get('CF-Connecting-IP'), `UA: ${UA}</tg-spoiler>\n域名: ${url.hostname}\n<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`);
					const 维列斯Config = await 生成配置信息(PID, request.headers.get('Host'), sub, UA, RPIP, url, env);
					const now = Date.now();
				...
				S5Add = url.searchParams.get('socks5') || S5Add;
				if (new RegExp('/socks5=', 'i').test(url.pathname)) S5Add = url.pathname.split('5=')[1];
				else if (new RegExp('/socks://', 'i').test(url.pathname) || new RegExp('/socks5://', 'i').test(url.pathname)) {
					S5Add = url.pathname.split('://')[1].split('#')[0];
					if (S5Add.includes('@')){
						let userPassword = S5Add.split('@')[0];
						const base64Regex = /^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i;
						if (base64Regex.test(userPassword) && !userPassword.includes(':')) userPassword = atob(userPassword);
						S5Add = `${userPassword}@${S5Add.split('@')[1]}`;
					}
				}
				...
				if (url.searchParams.has('Pip')){
					PIP = url.searchParams.get('Pip');
					enableS = false;
				} else if (new RegExp('/Pip=', 'i').test(url.pathname)) {
					PIP = url.pathname.toLowerCase().split('/Pip=')[1];
					enableS = false;
				} else if (new RegExp('/Pip.', 'i').test(url.pathname)) {
					PIP = `Pip.${url.pathname.toLowerCase().split("/Pip.")[1]}`;
					enableS = false;
				} else if (new RegExp('/pyip=', 'i').test(url.pathname)) {
					PIP = url.pathname.toLowerCase().split('/pyip=')[1];
					enableS = false;
				}
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log,) {
	async function useSocks5Pattern(address) {
		if ( go2S5s.includes(atob('YWxsIGlu')) || go2S5s.includes(atob('Kg==')) ) return true;
		return go2S5s.some(pattern => {
	...
	async function retry() {
		if (enableS) {
			...
			if (!PIP || PIP == '') {
				PIP = atob(`UFJPWFlJUC50cDEuZnh4ay5kZWR5bi5pbw==`);
			} else if (PIP.includes(']:')) {
				portRemote = PIP.split(']:')[1] || portRemote;
				PIP = PIP.split(']:')[0] || PIP;
			} else if (PIP.split(':').length === 2) {
				portRemote = PIP.split(':')[1] || portRemote;
				PIP = PIP.split(':')[0] || PIP;
			}
			if (PIP.includes('.tp')) portRemote = PIP.split('.tp')[1].split('.')[0] || portRemote;
			tcpSocket = await connectAndWrite(PIP || addressRemote, portRemote);
		}
	...
	let useSocks = false;
	if (go2S5s.length > 0 && enableS ) useSocks = await useSocks5Pattern(addressRemote);
	// 首次尝试连接远程服务器
 ...
 * @param {string} PID 用于验证的用户 ID
 * @returns {Object} 解析结果，包括是否有错误、错误信息、远程地址信息等
 */
function process维列斯Header(维列斯Buffer, PID) {
	...
	// 验证用户 ID（接下来的 16 个字节）
	function isUserIDValid(PID, userIDLow, buffer) {
		const userIDArray = new Uint8Array(buffer.slice(1, 17));
		const userIDString = stringify(userIDArray);
		return userIDString === PID || userIDString === userIDLow;
	}

	// 使用函数验证
	isValidUser = isUserIDValid(PID, userIDLow, 维列斯Buffer);
...
async function socks5Connect(addressType, addressRemote, portRemote, log) {
	const { username, password, hostname, port } = parsedS5Add;
	// 连接到 SOCKS5 代理服务器
...
const what_is_this = atob('ZG14bGMzTT0=');
function 配置信息(UUID, 域名地址) {
	const 协议类型 = atob(what_is_this);
 ...
 */
async function 生成配置信息(PID, hostName, sub, UA, RPIP, _url, env) {
	if (sub) {
		...
		if ((add.length + addapi.length + addnotls.length + addnotlsapi.length + addcsv.length) == 0){
			...
			add = add.concat('127.0.0.1:1234#CFnat');
			if (hostName.includes(".workers.dev")) {
				addnotls = addnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF随机节点'));
			} else {
				add = add.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF随机节点'));
			}
		}
	}

	const uuid = (_url.pathname == `/${FlowPIP}`) ? FlowPIP : PID;
	const userAgent = UA.toLowerCase();
	const Config = 配置信息(PID , hostName);
	const v2ray = Config[0];
	const clash = Config[1];
	let Phost = "";
	if(hostName.includes(".workers.dev")){
		if ( PhostsURL && (!Phosts || Phosts.length == 0)) {
			try {
				const response = await fetch(PhostsURL); 
				...
				Phosts = Phosts.concat(nonEmptyLines);
			} catch (error) {
				//console.error('获取地址时出错:', error);
			}
		} 
		if (Phosts.length != 0) Phost = Phosts[Math.floor(Math.random() * Phosts.length)] + "/";
	}

	if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
		const newSocks5s = S5s.map(S5Add => {
			if (S5Add.includes('@')) return S5Add.split('@')[1];
			else if (S5Add.includes('//')) return S5Add.split('//')[1];
			else return S5Add;
		});

		let socks5List = '';
		if (go2S5s.length > 0 && enableS ) {
			socks5List = `${decodeURIComponent('SOCKS5%EF%BC%88%E7%99%BD%E5%90%8D%E5%8D%95%EF%BC%89%3A%20')}`;
			if (go2S5s.includes(atob('YWxsIGlu'))||go2S5s.includes(atob('Kg=='))) socks5List += `${decodeURIComponent('%E6%89%80%E6%9C%89%E6%B5%81%E9%87%8F')}<br>`;
			else socks5List += `<br>&nbsp;&nbsp;${go2S5s.join('<br>&nbsp;&nbsp;')}<br>`;
		}

		let 订阅器 = '<br>';
		if (sub) {
			if (enableS) 订阅器 += `CFCDN（访问方式）: Socks5<br>&nbsp;&nbsp;${newSocks5s.join('<br>&nbsp;&nbsp;')}<br>${socks5List}`;
			else if (PIP && PIP != '') 订阅器 += `CFCDN（访问方式）: ProxyIP<br>&nbsp;&nbsp;${PIPs.join('<br>&nbsp;&nbsp;')}<br>`;
			else if (RPIP == 'true') 订阅器 += `CFCDN（访问方式）: 自动获取ProxyIP<br>`;
			else 订阅器 += `CFCDN（访问方式）: 无法访问, 需要您设置 PIP/PROXYIP ！！！<br>`
			...
			订阅器 += `<br>您的订阅内容由 内置 add/ADD* 参数变量提供${判断是否绑定KV空间}<br>`;
			if (add.length > 0) 订阅器 += `ADD（TLS优选域名&IP）: <br>&nbsp;&nbsp;${add.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addnotls.length > 0) 订阅器 += `ADDNOTLS（noTLS优选域名&IP）: <br>&nbsp;&nbsp;${addnotls.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addapi.length > 0) 订阅器 += `ADDAPI（TLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addapi.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addnotlsapi.length > 0) 订阅器 += `ADDNOTLSAPI（noTLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addnotlsapi.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addcsv.length > 0) 订阅器 += `ADDCSV（IPTest测速csv文件 限速 ${DLS} ）: <br>&nbsp;&nbsp;${addcsv.join('<br>&nbsp;&nbsp;')}<br>`;
		}

		if (FlowPIP && _url.pathname !== `/${FlowPIP}`) 订阅器 = '';
		else 订阅器 += `<br>SUBAPI（订阅转换后端）: ${subProtocol}://${subConver}<br>SUBCONFIG（订阅转换配置文件）: ${subConfig}`;
		const FlowPIP信息 = (uuid != PID) ? `TOKEN: ${uuid}<br>UUIDNow: ${PID}<br>UUIDLow: ${userIDLow}<br>${userIDTime}TIME（FlowPIPenabletime）: ${enabletime} 天<br>UPTIME（FlowPIPupdatetime）: ${updatetime} 时（北京时间）<br><br>` : `${userIDTime}`;
		const 节点配置页 = `
			################################################################<br>
			Subscribe / sub 订阅地址, 支持 Base64、clash-meta、sing-box 订阅格式<br>
			---------------------------------------------------------------<br>
			自适应订阅地址:<br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}</a><br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?sub')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?sub</a><br>
			<br>
			Base64订阅地址:<br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?b64')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?b64</a><br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?base64')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?base64</a><br>
			<br>
			clash订阅地址:<br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?clash')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?clash</a><br>
			<br>
			singbox订阅地址:<br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?sb')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?sb</a><br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${Phost}${hostName}/${uuid}?singbox')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${Phost}${hostName}/${uuid}?singbox</a><br>
			<br>
			<strong><a href="javascript:void(0);" id="noticeToggle" onclick="toggleNotice()">实用订阅技巧∨</a></strong><br>
				<div id="noticeContent" class="notice-content" style="display: none;">
					...
					&nbsp;&nbsp;https://${Phost}${hostName}/${uuid}<strong>?sub=sub.google.com</strong><br>
					<br>
					<strong>3.</strong> 快速更换 PROXYIP 至：Pip.fxxk.dedyn.io:443，您可将"?Pip=proxy.fxxk.dedyn.io:443"参数添加到链接末尾，例如：<br>
					&nbsp;&nbsp; https://${Phost}${hostName}/${uuid}<strong>?Pip=proxyip.fxxk.dedyn.io:443</strong><br>
					<br>
					<strong>4.</strong> 快速更换 SOCKS5 至：user:password@127.0.0.1:1080，您可将"?socks5=user:password@127.0.0.1:1080"参数添加到链接末尾，例如：<br>
					&nbsp;&nbsp;https://${Phost}${hostName}/${uuid}<strong>?socks5=user:password@127.0.0.1:1080</strong><br>
					<br>
					<strong>5.</strong> 如需指定多个参数则需要使用'&'做间隔，例如：<br>
					&nbsp;&nbsp;https://${Phost}${hostName}/${uuid}?sub=sub.google.com<strong>&</strong>Pip=proxyip.fxxk.dedyn.io<br>
				</div>
			<script>
		...
		if (hostName.includes(".workers.dev")){
			...
			newAddressesnotlsapi = await 整理优选列表(addnotlsapi);
		...
		let url = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${fakePID + atob('JmVkZ2V0dW5uZWw9Y21saXUmcHJveHlpcD0=') + RPIP}&path=${encodeURIComponent(path)}`;
		let isBase64 = true;
		...
		if (!sub || sub == ""){
			if(hostName.includes('workers.dev')) {
				...
				// 使用Set对象去重
				Phosts = [...new Set(Phosts)];
			}
	
			newAddressesapi = await 整理优选列表(addapi);
			newAddressescsv = await 整理测速结果('TRUE');
			url = `https://${hostName}/${fakePID + _url.search}`;
		...
		if (!userAgent.includes(('CF-Workers-SUB').toLowerCase())){
			if ((userAgent.includes('clash') && !userAgent.includes('nekobox')) || ( _url.searchParams.has('clash') && !userAgent.includes('subconverter'))) {
				url = `${subProtocol}://${subConver}/sub?target=clash&url=${encodeURIComponent(url)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=${subEmoji}&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
				isBase64 = false;
			} else if (userAgent.includes('sing-box') || userAgent.includes('singbox') || (( _url.searchParams.has('singbox') || _url.searchParams.has('sb')) && !userAgent.includes('subconverter'))) {
				url = `${subProtocol}://${subConver}/sub?target=singbox&url=${encodeURIComponent(url)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=${subEmoji}&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
				isBase64 = false;
			}
		}
		...
		try {
			...
			if ((!sub || sub == "") && isBase64 == true) {
				content = await 生成本地订阅(fakeHostName,fakePID,noTLS,newAddressesapi,newAddressescsv,newAddressesnotlsapi,newAddressesnotlscsv);
			} else {
			...
			if (_url.pathname == `/${fakePID}`) return content;

			return 恢复伪装信息(content, PID, hostName, isBase64);
				...
				if (lines[0].split(',').length > 3){
					...
					for (let i = 1; i < lines.length; i++) {
						...
						if(columns){
							...
							if (api[index].includes('Pip=true')) PIPPool.push(`${columns}:${测速端口}`);
						}
					}
				} else {
					// 验证当前apiUrl是否带有'Pip=true'
					if (api[index].includes('Pip=true')) {
						// 如果URL带有'Pip=true'，则将内容添加到PIPPool
						PIPPool = PIPPool.concat((await 整理(content)).map(item => {
...
async function 整理测速结果(tls) {
	if (!addcsv || addcsv.length === 0) {
		return [];
	}
	
	let newAddressescsv = [];
	
	for (const csvUrl of addcsv) {
		try {
			...
			for (let i = 1; i < lines.length; i++) {
				...
				if (columns[tlsIndex].toUpperCase() === tls && parseFloat(columns[speedIndex]) > DLS) {
					...
					if (csvUrl.includes('Pip=true') && columns[tlsIndex].toUpperCase() == 'true' && !httpsPorts.includes(port)) {
						// 如果URL带有'Pip=true'，则将内容添加到PIPPool
						PIPPool.push(`${ipAddress}:${port}`);
					}
				}
			}
		} catch (error) {
...
function 生成本地订阅(host,UUID,noTLS,newAddressesapi,newAddressescsv,newAddressesnotlsapi,newAddressesnotlscsv) {
	...
	add = add.concat(newAddressesapi);
	add = add.concat(newAddressescsv);
	let notlsresponseBody ;
	if (noTLS == 'true'){
		addnotls = addnotls.concat(newAddressesnotlsapi);
		addnotls = addnotls.concat(newAddressesnotlscsv);
		const uniqueAddressesnotls = [...new Set(addnotls)];
	...
	// 使用Set对象去重
	const uniqueAddresses = [...new Set(add)];
	...
	const responseBody = uniqueAddresses.map(address => {
		...
		let 最终路径 = path ;
		let 节点备注 = '';
		const matchingProxyIP = PIPPool.find(PIP => PIP.includes(address));
		if (matchingProxyIP) 最终路径 += `&Pip=${matchingProxyIP}`;
		
		if(Phosts.length > 0 && (伪装域名.includes('.workers.dev'))) {
			最终路径 = `/${伪装域名}${最终路径}`;
			伪装域名 = Phosts[Math.floor(Math.random() * Phosts.length)];
...
function creatFlowPIP(key) {
	...
	// 生成两个 UUID
	const 当前UUIDPromise = 生成UUID(key + 当前周数);
	const 上一个UUIDPromise = 生成UUID(key + (当前周数 - 1));
